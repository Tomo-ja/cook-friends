...
export const Form = ({ btn, signUp, }: props) => {
	...
	const connectApi = (e: React.MouseEvent<HTMLElement>) => {
		...
		if (btn === "Sign up") {
			...
			appAxios.post("api/auth/register", { data: Ref }).then((res) => {
				if (res.data === "exsist") {
					return setErr({ ...err, password: true, validation: true, account: false });
				} else {
					router.push("/login");
				}
			});
			...
			appAxios.post("api/auth/login", { data: Ref }).then((res) => {
				if (res.data === "NotExists") {
					return setErr({ password: true, validation: true, account: true, login: false, loginPsw: false });
				} else if (res.data === "worngPassword") {
					return setErr({ password: true, validation: true, account: true, login: true, loginPsw: false });
				} else {
					const cookies = parseCookies();
					console.log({cookies});
					console.log(res.data.user._id);
					console.log(res.data.user);
					setCookie(null, "accessToken", JSON.stringify(res.data), {
						maxAge: 30 * 24 * 60 * 60,
						path: "/",
					});
					// const user = res.data;
	...
	return (
		<FormStyled>
			...
			{!signUp && <p> You don&apos;t have an account yet ? <Link href="/signup" className="signup">Signup</Link> </p>}
		</FormStyled>
	);
};
